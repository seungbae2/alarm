...
class UpdateAlternatingAlarmUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            ...
            val endTimestamp = endLocalDate.atTime(23, 59, 59).atZone(ZoneId.systemDefault()).toEpochSecond() * 1000
            ...
            alternatingSteps.forEach { step ->
                repeat(step.durationDays) { dayIndex ->
                    step.times.forEach { alarmTime ->
                        ...
                        val alarmStartTimestamp = alarmStartDate.atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000
